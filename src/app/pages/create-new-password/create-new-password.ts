  ...
  ngOnInit(): void {
    ...
    this.authService.validateResetToken(this.token).subscribe({
      ...
      error: (error) => {
        ...
        this.errorMessage = apiMessage(error, 'This password reset link is invalid or expired.');
      },
    });
  }
  ...
  submit(): void {
    ...
    this.authService.resetPassword(this.token, data).subscribe({
      next: (response) => {
        ...
        setTimeout(() => {
          this.router.navigate(['/reset-password-successfully']);
        }, 700);
      },
      ...
      error: (error) => {
        ...
        this.errorMessage = mappedMessage || apiMessage(error, 'Unable to reset password.');
      },
    });
  }
}
