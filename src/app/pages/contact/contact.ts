...
export class Contact {
  ...
  form = this.fb.nonNullable.group({
    ...
    email: ['', [Validators.required, Validators.pattern(/^[a-z]{3,20}[0-9]{0,12}@gmail\.com$/)]],
  ...
  submit(): void {
    ...
    console.log('[Contact Page] Sending contact request:', data);

    this.userService.sendContact(data).subscribe({
      next: (response) => {
        console.log('[Contact Page] Contact success:', response);
      ...
      error: (error) => {
        console.error('[Contact Page] Contact API error:', error);

        this.loading = false;

        const mappedMessage = applyBackendErrors(this.form, error);

        this.errorMessage = mappedMessage || apiMessage(error, 'Unable to send your message.');
      },
    });
  }
}
