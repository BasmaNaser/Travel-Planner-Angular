...
import { BookingService, MyBooking } from '../../services/booking.services';
...
export class Profile implements OnInit {
  ...
  form = this.fb.group({
    fullName: this.fb.nonNullable.control('', [
      Validators.required,
      Validators.pattern(/^[a-zA-Z]{3,20}(( )[a-zA-Z]{3,20}){1,4}$/),
      // Validators.pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@%$&*^#]).{8,}$/),
    ]),

    gender: this.fb.control<'Male' | 'Female' | 'Other' | undefined>(undefined),

    about: this.fb.nonNullable.control('', [Validators.maxLength(1000)]),
  ...
  uploadProfilePicture(): void {
    ...
    this.userService.uploadProfilePicture(this.selectedImage).subscribe({
      next: (response: any) => {
        ...
        this.successMessage = response?.message || 'Profile picture updated successfully.';
      },

      error: (error: any) => {
        this.uploadingImage = false;

        this.errorMessage = error?.error?.message || 'Unable to upload profile picture.';
      },
    });
  }
  ...
  deleteProfilePicture(): void {
    ...
    this.userService.deleteProfilePicture().subscribe({
      next: (response: any) => {
        ...
        this.successMessage = response?.message || 'Profile picture deleted successfully.';
      },

      error: (error: any) => {
        this.deletingImage = false;

        this.errorMessage = error?.error?.message || 'Unable to delete profile picture.';
      },
    });
  }
  ...
  deleteAccount(): void {
    ...
    this.userService.deleteAccount().subscribe({
      next: (response: any) => {
        this.successMessage = response?.message || 'Account deleted successfully.';
      },

      error: (error: any) => {
        this.errorMessage = error?.error?.message || 'Unable to delete account.';
      },
    });
  }
  ...
  changePassword(): void {
    ...
    if (!confirmPassword) {
      this.passwordErrorMessage = 'Please confirm your new password.';
  ...
  loadBookings(): void {
    ...
    this.bookingService.getMyBookings().subscribe({
      ...
      error: (error: any) => {
        ...
        this.bookingError = error?.error?.message || 'Unable to load your bookings.';
  ...
  getBookingDestination(booking: MyBooking): any {
    return booking?.ItineraryDayID?.DestinationID ?? {};
  }
